...
export function applyBordersFrayedAndGrungeHoisted(parsedSettings, ctx) {
  const { borderWidth, borderPosition, borderColorOption, borderColorHex, frayedBorder, frayedBorderBlurLevel, frayedBorderSize, frayedBorderSeed, drawGrunge, grungeWidth, frayedBorderColorHex, mergeColor: mergeColorFromCtx } = ctx
...
export function applyRoadsAndScalesHoisted(parsedSettings, ctx) {
  const { drawRoads, roadStyle, roadWidth, roadColorHex, mountainSize, hillSize, duneSize, treeHeight, citySize, scaleSliderValue, mergeColor: mergeColorFromCtx } = ctx
  const merge = mergeColorFromCtx || mergeColor
...
export function applyTextAndBackgroundHoisted(parsedSettings, ctx) {
  const { drawText, textColorHex, drawBoldBackground, boldBackgroundColorHex, mergeColor: mergeColorFromCtx } = ctx
  const merge = mergeColorFromCtx || mergeColor
