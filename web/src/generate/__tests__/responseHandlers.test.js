...
describe('responseHandlers', () => {
  ...
  it('handleSuccess sets preview and current source', () => {
    ...
    const state = {}
    const setPreview = vi.fn((fn) => { state.preview = fn(null); return state.preview })
    const setCurrentSource = vi.fn((v) => { state.current = v; return v })

    handleSuccess(blob, { baseName: 'base', source: { type: 'random' }, fileName: 'f', setPreview, setCurrentSource })
  ...
  it('processGenerateResponse handles non-json image bytes', async () => {
    ...
    await processGenerateResponse(bytes, 'image/png', { outputMode: 'image', setPreview, setCurrentSource })
    expect(setPreview).toHaveBeenCalled()
  })
})
