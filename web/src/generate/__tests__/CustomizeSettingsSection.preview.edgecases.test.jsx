...
test('composeMiniIslandFromBlobModule resolves to null when canvas.toBlob yields null', async () => {
  ...
  vi.stubGlobal('createImageBitmap', vi.fn(async () => ({ width: 64, height: 64 })));
  ...
  const makeCanvasForBitmap = (imgBitmap) => {
    const canvas = {
      ...
      getContext() {
        return {
          save() {}, restore() {}, drawImage() {}, beginPath() {}, closePath() {},
          moveTo() {}, lineTo() {}, clip() {}, fillRect() {}, createPattern: () => null,
          fill() {}, stroke() {}, globalCompositeOperation: 'source-over',
        }
      }
    }
  ...
  const result = await composeMiniIslandFromBlobModule(new Blob(['x'], { type: 'image/png' }), {}, {}, {}, overrides)
...
test('composeMiniIslandFromBlobModule works when pattern is absent (no throw)', async () => {
  ...
  const makeCanvasForBitmap = (imgBitmap) => {
    const canvas = {
      width: imgBitmap.width,
      height: imgBitmap.height,
      toBlob(cb) { cb(new Blob(['ok'], { type: 'image/png' })) },
      getContext() {
        return {
          save() {}, restore() {}, drawImage() {}, beginPath() {}, closePath() {},
          moveTo() {}, lineTo() {}, clip() {}, fillRect() {}, createPattern: () => null,
