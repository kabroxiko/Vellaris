...
/**
 * Batch test that exercises multiple MapApiServer helpers in a single JVM invocation.
 * Run this when you want the focused tests to execute together without repeated Gradle invocations.
 */
class MapApiServerBatchTest
{
    ...
    void runFocusedMapApiServerChecks() throws Exception
    {
        ...
        assertNotNull(img);
        if (img instanceof AutoCloseable) ((AutoCloseable) img).close();
        ...
        if (baseImg instanceof AutoCloseable) ((AutoCloseable) baseImg).close();
