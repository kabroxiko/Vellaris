...
public class SubMapCreatorTest
{
	...
	/**
	 * Asserts that all rivers form a single connected component, i.e. every river shares at least one corner with some other river. Uses
	 * union-find to detect disconnected river arms.
	 */
	...
	/**
	 * Asserts that no corner in the combined river edge graph has degree &gt; 2. Such corners are branch points, indicating a finger was
	 * introduced. Note: {@link WorldGraph#findRivers()} may place the same edge in two different {@link River} objects, so edges are
	 * deduplicated before computing degrees.
	 */
