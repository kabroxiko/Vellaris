...
/**
 * Test fixtures helper to install test-only MapCreator factories. This class is compiled only into the test fixtures output and used by
 * tests to avoid adding any test hooks to production code.
 */
public class MapApiServerTestHooks
{
	public static void setMapCreatorFactory(java.util.function.Supplier<nortantis.MapCreator> factory)
	{
		try
		{
			...
			{
				java.util.concurrent.atomic.AtomicReference<java.util.function.Supplier<nortantis.MapCreator>> ref = new java.util.concurrent.atomic.AtomicReference<>(
						factory != null ? factory : (java.util.function.Supplier<nortantis.MapCreator>) nortantis.MapCreator::new);
				f.set(null, ref);
			}
		}
