...
/**
 * Small command-line utility to generate maps headless.
 * Usage (after building jar):
 *  java -cp build/libs/Nortantis.jar nortantis.cli.MapGeneratorCLI --random --out out.png --width 2000 --height 1200
 *  java -cp build/libs/Nortantis.jar nortantis.cli.MapGeneratorCLI --nort mymap.nort --out out.png
 */
public class MapGeneratorCLI {
    ...
    private static Config parseArgs(String[] args)
    {
        ...
        handlers.put("--nort", it2 -> { if (it2.hasNext()) cfg.nortFile = it2.next(); });
        handlers.put("--out", it2 -> { if (it2.hasNext()) cfg.outPath = it2.next(); });
        handlers.put("--width", it2 -> { if (it2.hasNext()) cfg.width = Integer.parseInt(it2.next()); });
        handlers.put("--height", it2 -> { if (it2.hasNext()) cfg.height = Integer.parseInt(it2.next()); });
        handlers.put("--seed", it2 -> { if (it2.hasNext()) cfg.seed = Long.parseLong(it2.next()); });
